...
9. 最后将三路并联模型融合，并采用Dense层，压缩成对应6个分类标签。
   ...
   dense1 = Dense(100, activation='relu')(conv_sum)  # 压缩成对应6个标签
...
- epoch
根据提供的benchmark model等参考模型，通常epoch取值为2~3。但在计算发现，随着epoch的增大，计算得分提升较大。但通常超过5后，计算得分会降低。且随着epoch的增大，运算时间也会倍数上升。因此最终选择epoch = 5.
